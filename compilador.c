...
{
    opcode[8] = '\0';
    if (strcasecmp(opcode, "10001000") == 0)
    {
        // funcao mover reg / reg memo
        moveR(registradores, opcode1);
    }
    else if (strcasecmp(opcode, "11000110") == 0)
    {
        // funcao mover reg/ i
        moveI(registradores, opcode1);
    }
    else if (strcasecmp(opcode, "00000000") == 0)
    {
        // funcao add reg / reg
        addR(registradores, opcode1);
    }
    else if (strcasecmp(opcode, "10000000") == 0)
    {
        // funcao addi reg / i
        addI(registradores, opcode1);
    }
    else if (strcasecmp(opcode, "00101000") == 0)
    {
        // funcao sub reg / reg
        subR(registradores, opcode1);
    }
    else if (strcasecmp(opcode, "10000010") == 0)
    {
        // funcao sub reg / i
        subI(registradores, opcode1);
    }
    else if (strcasecmp(opcode, "11110110") == 0)
    {
        // funcao mul reg /reg
    ...
    else if (strcasecmp(opcode, "00100000") == 0)
    {
        // funca and reg / reg
        funcaoAnd(registradores, opcode1);
    }
    else if (strcasecmp(opcode, "00001000") == 0)
    {
        // funcao or reg /reg  101 001 001
        funcaoOr(registradores, opcode1);
    }
    else if (strcasecmp(opcode, "00110000") == 0)
    {
        // funcao xor reg / reg
        funcaoXor(registradores, opcode1);
    }
    else if (strcasecmp(opcode, "11010000") == 0)
    {
        // funcao sll shl
        funcaoShl(registradores, opcode1);
    }
    else if (strcasecmp(opcode, "11010010") == 0)
    {
        // funcao srl shr
...
int verificaSyscall(char *opcode)
{
    ...
    if (strcasecmp(opcode, "11001101") == 0)
    {
        valor = 1;
    }
...
{
    int valor = 0; // definido como falso por padrao
    if (strcasecmp(opcode, "11100011") == 0)
    {
        valor = 1;
    }
...
{
    int valor = 0; // definido como falso por padrao
    if (strcasecmp(opcode, "11101001") == 0)
    {
        valor = 1;
    }
...
{
    int valor = 0; // definido como falso por padrao
    if (strcasecmp(opcode, "01110100") == 0)
    {
        valor = 1;
    }
...
{
    int valor = 0; // definido como falso por padrao
    if (strcasecmp(opcode, "01110101") == 0)
    {
        valor = 1;
    }
...
{
    int valor = 0; // definido como falso por padrao
    if (strcasecmp(opcode, "00111000") == 0)
    {
        valor = 1;
    }
...
void iniciaLabel(Label *lista, int qtd)
{
    for (int i = 0; i < qtd; i++)
    {
...
int pularNoArquivo(FILE *arq, Label *listaLabel, int qtdLabel, char *opcode1, int linhaAtual)
{
    ...
    int valorAchado = 0;
    int find = 0; // auxiliar segundo encontrou
    int valor = 0;
    ...
    {
        if (strcasecmp(listaLabel[j].conteudo, aux) == 0)
        {
            Encontrou = 1;
            valorAchado = j;
        }
    ...
    else
    {
        ...
        while (fgets(linha, sizeof(linha), arq) != NULL) // procurando a label
        {
            ...
            {
                if (strcasecmp(linha, aux) == 0) // verifica se é a mesma
                {
                    break;
                }
                else
